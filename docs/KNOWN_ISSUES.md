...
| KI-003 | Audio | Upstream tree has no audio output device code (CoreAudio/SDL-audio/AudioQueue absent; musyx/amuse playback calls commented out in `CSfxManager.cpp`/`CStreamAudioManager.cpp`/`CMidiManager.cpp`; the `boo` voice engine is commented out in `CMain.cpp`). The musyx synthesis library is vendored and complete; the host integration (decode → mix → device) was missing. | **FIXED (2026-08-11, local patch)** — implemented a minimal SDL3 audio output module in aurora (`extern/aurora/include/aurora/audio.h` + `lib/audio.cpp`, registered in `aurora_core.cmake`), restored the frontend music plumbing in `CStaticAudioPlayer` (RSF load via `CDvdFile`, G721 decode in a per-frame supply callback) and `CFrontEndUI` (audio1/audio2 created, `StartMixing`/`StopMixing` on screen transitions), and wired `aurora_audio_pump()` into `CMain::onAppPostDraw` + `aurora_audio_close()` into `onAppExiting`. **Verified on macOS:** SDL opens the default output device (converted to 44100 Hz stereo, 1024-sample frames); `Audio/frontend_1.rsf` (1,973,676 B) and `Audio/frontend_2.rsf` (1,636,982 B) load and decode; the pump supplies 533/534 frames per 60 Hz frame with a steady stream buffer (25–34 KB, no underrun/overflow across a multi-minute run); title/attract runs at 60 FPS, 13.6% CPU; window-close exits cleanly with no crash report. Two SDL3 API notes: default device is `SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK` (0xFFFFFFFF, not 0), and `SDL_CreateAudioStream` takes src+dst specs. Patch: `patches/2026-08-11-metaforce-enable-audio-output.patch` (first three file entries apply inside `extern/aurora`, the rest in the metaforce root). Scope note: this covers the frontend/static audio path; musyx-driven game SFX/music playback (`CSfxManager`/`CStreamAudioManager`/`CMidiManager`) is still commented out upstream and remains the next audio gap for in-game sound. | Logs `/tmp/mf_audio3.log`, `/tmp/mf_audio4.log`; screenshot `/tmp/mf_audio_final.png` (Samus intro, 60 FPS) |
