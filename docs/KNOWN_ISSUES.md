...
| KI-011 | Saves (open) | After KI-009, the new game creates its save slot correctly in-memory, but the **kabufuda raw card file on disk is not updated** (mtime unchanged across runs even after a clean SIGINT exit; `CMemoryCardSys::CommitToDisk` → `Card::commit()` uses async writes that never flush, or the driver never marks the card dirty). Saves therefore do not survive a relaunch yet. | Open — card persistence needs investigation (kabufuda async I/O / commit path on macOS) | 2026-08-11; card file `~/Library/Application Support/dolphin-emu/GC/MemoryCardA.USA.raw` unchanged |
