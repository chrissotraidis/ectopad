...
| KI-022 | First beam door appears not to open | The first tutorial door remained visible after being shot even though the player could walk through it. A later workaround simply made it disappear. | **Open; narrowed to animated-model presentation.** Projectile, Open state, and collision transition work. The authored 0.833333-second animation exists, type 0 is selected, and `IsAnimating()` is true while the closed model remains visible. The abrupt-hide workaround remains removed; diagnostics only are deployed. | `Runtime/World/CScriptDoor.cpp`; `/tmp/ectopad-door-animation.Mv88YO/runtime.log`; `patches/2026-08-13-metaforce-door-animation-diagnostics.patch` | 2026-08-13 |
| KI-023 | World rendering / black distance geometry and actors | During Frigate play, large world polygons render black or incorrectly translucent; enemies can appear down/dead at distance while alive and shooting. | **Open; isolated lighting candidate deployed, not accepted.** The four-change bundle was rolled back. A subsequent physical build containing only the unbound-texture fix still rendered the same black geometry and door, so that fix was removed. The final build contains only upstream Aurora `2bbb122` for correct GX channel diffuse-function encoding; its targeted test and all 165 GX tests pass. Physical visual acceptance is deferred. | `patches/2026-08-13-aurora-gx-channel-lighting.patch`; `/tmp/ectopad-door-black-2026-08-13.mov`; screenshot `Screenshot 2026-08-13 at 3.22.24 PM.png` | 2026-08-13 |
| KI-024 | Later-game stability / Morph Ball | File slot 2 loaded, but entering Morph Ball and moving toward a tunnel immediately ended the play session. The launched PID was 1812; afterward EctoPad was resident as PID 1826. iOS produced no crash, jetsam, hang, or GPU report, and the original runtime log stopped abruptly without a lifecycle notification. | **P0 open; file slot 2 is not accepted as playable.** The copied card also loads on macOS, ruling out basic card/CRC corruption, but its active-combat checkpoint is a poor deterministic fixture. The four-patch renderer experiment is rolled back. Engine breadcrumbs now write synchronously and log every Morph Ball state transition. | `/tmp/ectopad-morph-crash.GNrkCN/runtime-current.log`; `/tmp/ectopad-morph-crash.GNrkCN/Metaforce-live.trace`; physical report 2026-08-13 | 2026-08-13 |
