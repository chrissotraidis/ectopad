...
| KI-003 | Audio | No audio output device code in the current tree (CoreAudio/SDL-audio/AudioQueue absent; musyx/amuse playback calls commented out in `CSfxManager.cpp`/`CStreamAudioManager.cpp`/`CMidiManager.cpp`; the `boo` voice engine is commented out in `CMain.cpp`). The musyx synthesis library is vendored and complete (sndInit/voices/rooms/emitters); the host integration (decode → mix → device) is missing. Audio assets load without error. Implementation plan in `IMPLEMENTATION_PLAN.md` §Audio. | Open — upstream mid-refactor gap (documented plan) | Tree inspection 2026-08-11 |
