...
| KI-003 | Audio | Upstream tree had no output device and its amuse game-audio paths were removed; the first local SDL pump then produced exactly one 60 Hz slice per rendered frame, so any render dip could drain the device faster than it was refilled. | **Implementation complete; audible mobile acceptance open.** The existing local patches restore SDL output, frontend RSF, amuse voices, streamed DSP/MIDI, and soxr pitch conversion. On 2026-08-12 the SunPad audio investigation was audited: its primary 12× static-recompiler guest-timebase bug is not applicable to native Metaforce, but its bounded-reserve/no-stale-replay policy is. Aurora now measures output-ready audio after SDL conversion and tops a bounded 120 ms reserve with fresh samples, independent of render FPS. Final macOS, iOS Simulator, and iPhoneOS arm64 builds pass; a loaded native run held 5,292–5,294/5,292 output frames with 6–7 active voices and no underrun log despite heavy competing renderer load. Physical speaker quality, interruptions/routes, and long-session acceptance remain required. | Patch `patches/2026-08-12-aurora-frame-rate-independent-audio-reserve.patch`; log `/tmp/metaforce-audio-reserve-native-2026-08-12.log`; prior audio evidence `/tmp/mf_soxr.log`, `/tmp/mf_midi.log`, `/tmp/mf_ios_sim_audio.log` |
...
| KI-015 | iOS touch overlay attach regression | The iOS build compiled both `lib/ios_touch_stub.cpp` and `lib/ios/OverlayBridge.mm` into the same archive; archive order selected the stub and omitted the real Objective-C++ bridge. | **FIXED (2026-08-12).** CMake now selects one implementation. The current bridge renders the unchanged SunPad layer; a guarded current-build harness passes all menu/control/settings/editor/Files phases (`result=0`), and the live bridge reports `menu visible=1 attached=1`. Because SDL may later change its UIKit/Metal hierarchy, the bridge now reasserts the whole SunPad overlay at window front once per second. Host click delivery remains unreliable, so physical finger/multitouch proof is still required. | Patches `2026-08-12-aurora-sunpad-touch-overlay.patch` and `2026-08-12-aurora-sunpad-interaction-menu-visibility.patch`; evidence `/tmp/ipad-menu-audio-focused-2026-08-12.{png,log}` | 2026-08-12 Simulator |
