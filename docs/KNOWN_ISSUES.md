...
| KI-015 | iOS touch overlay attach regression | The iOS build compiled both `lib/ios_touch_stub.cpp` and `lib/ios/OverlayBridge.mm` into the same static `libaurora_core.a`. `ios_touch_stub.cpp` did not include SDL's platform definitions, so its preprocessor guard incorrectly emitted the non-iOS `attach_ios_overlay()` implementation. Because that object preceded `OverlayBridge.mm.o` in archive order, the static linker satisfied Aurora's reference from the stub and never loaded the real Objective-C++ bridge. This produced only Aurora's generic warning and none of the bridge's detailed `NSLog` messages; SDL's `UIWindow` property timing was not the cause. | **FIXED (2026-08-12)** — CMake now compiles `ios_touch_stub.cpp` only for non-iOS builds; the stub itself is unconditional so platform selection has one owner. Rebuilt both `ios-sim` and `macos-default-relwithdebinfo`. The iPad Simulator build logged `[SunPad] session start`, showed the unchanged SunPad controls over live intro/game rendering, reached Dawn→Metal, and terminated with exit 0. Current Simulator host clicks still fail to deliver touch, so opening the menu/settings on this exact build is blocked by test infrastructure; earlier menu rendering remains screenshot-proven. Patch: `patches/2026-08-12-aurora-sunpad-touch-overlay.patch`. | Current: `/tmp/ki015-overlay-fixed-2026-08-12.png`, `/tmp/ki015-overlay-fixed-2026-08-12.log`; earlier menu: `/tmp/ios_menu_open2.png`; parity: `docs/SUNPAD_PARITY.md` | 2026-08-12 Simulator |
