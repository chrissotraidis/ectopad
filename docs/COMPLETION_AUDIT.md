...
| Strict SunPad overlay/menu/touch parity | Overlay/settings/mixer/input/mapping sources byte-identical; diagnostics has app-name-only changes; current-build harness passes menu/control/settings/editor/picker paths and live bridge reports `•••` visible/attached | **Source/presentation and deterministic UIKit wiring proven; physical interaction partial** | Finger/multitouch menu/editor/gameplay tests on physical iPhone + iPad |
